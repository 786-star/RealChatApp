...
const SignUp = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true)
        try {
            const res = await baseUrl.post('/auth/create', userForm)
            console.log(res)
        ...
        <div className='w-full h-screen bg-slate-200 flex items-center justify-center'>
            <div className='w-full max-w-125 h-150 bg-white rounded-lg shadow-gray-400 shadow-lg flex flex-col gap-7.5'>
                ...
                <form className='flex items-center w-full flex-col gap-5' onSubmit={handleSubmit}>
                    <input type="text" placeholder='Enter username' className={InputStyle} value={userForm.userName} onChange={handleFormChange} name="userName" />
                    ...
                    <button type='submit' className={btnStyle}>{loading ? "Loading..." : "Sign Up"}</button>
